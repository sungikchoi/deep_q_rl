...
class OptimalityTightening(object):
    ...
    def _do_training(self):
        ...
        #print "=" * 50
        #print self.step_counter, self.batchnum, self.batch_count
        if self.batchnum == self.batch_count:
            self.y2 = np.array([])
            self.epi_state, self.epi_actions, self.epi_rewards, self.batchnum, self.epi_terminals = self.data_set.random_batch()
            self.epi_len = self.batchnum * self.batch_size			
			
            for i in range(self.batchnum):
                self.y2 = np.append(self.y2, self.network.q_target_vals(self.epi_state[self.batch_size * i:self.batch_size * (i+1)]))
            self.y2 = np.reshape(self.y2, (self.batchnum * self.batch_size, self.num_actions))
            self.y2 = np.roll(self.y2, self.num_actions)
            for i in range(self.epi_len):
                if self.epi_terminals[i]:
                    self.y2[i,:] = [0]*self.num_actions
					
            self.a = np.zeros((self.epi_len, self.num_actions))
            self.y_  = np.zeros(self.epi_len,dtype=np.float32)
			
            for i in range(0, self.epi_len):
                if i < self.epi_len - 1:
                    self.a[i, self.epi_actions[i]] = 1
                    if self.epi_terminals[i]:
                        self.y_ [i] = self.epi_rewards[i]
                        self.y2[i+1,self.epi_actions[i]] = 0.5*self.y_ [i]+0.5*self.y2[i+1,self.epi_actions[i]]
                    elif self.epi_terminals[i+1]:
                        self.y_ [i] = self.epi_rewards[i] + self.discount * np.max(self.y2[i])
                        self.y2[i+1,:] = 0
                    else:
                        self.y_ [i] = self.epi_rewards[i] + self.discount * np.max(self.y2[i])
                        self.y2[i+1, self.epi_actions[i]] = 0.5*self.y_ [i]+0.5*self.y2[i+1,self.epi_actions[i]]
            ...
            self.batch_count = 1
            #print "new episode sampled: ",self.step_counter, self.batchnum, self.batch_count
            ...
            self.batch_count += 1
            #print "update previously sampled episode: ", self.step_counter, self.batchnum, self.batch_count
    ...
    def finish_epoch(self, epoch):
        ...
        #print "Average training score: " + str(float(self.total_reward)/self.episode_counter)
        #self.total_reward = 0
        #self.episode_counter = 0
